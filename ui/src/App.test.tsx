...
describe("App frontend FE smoke", () => {
  ...
  it("covers FE6 happy path: reference -> generate zones -> step 2", async () => {
    ...
    expect(screen.getByText(/selecao zona:/i)).toBeInTheDocument();
  });
  ...
  it("verifies M5.7: cache hit under 500ms and incremental diff without list flicker", async () => {
    ...
    await user.click(screen.getByRole("button", { name: "Zonas e Imóveis" }));
    ...
    await user.click(screen.getByRole("button", { name: /Zona 1/i }));
    ...
    const stableCardBefore = screen.getByText(/Rua A, 100/i).closest("li");
    expect(stableCardBefore).not.toBeNull();
    ...
    const stableCardAfter = screen.getByText(/Rua A, 100/i).closest("li");
    expect(stableCardAfter).toBe(stableCardBefore);
  });

  it("verifies M6.2: dashboard tab loads with 30-day line chart data for FREE", async () => {
    const user = userEvent.setup();
