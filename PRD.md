...
> **Observação de progresso (2026-05-27, scraper Loft):** adicionada a plataforma Loft ao scraping de imóveis com adapter Playwright/Next.js, registro em `platforms.yaml`, registry de plataformas, constantes de plataforma, scripts de verificação e testes focados de extração/registry. Documentos obrigatórios abertos nesta execução: `PRD.md`, `SKILLS_README.md`, `skills/best-practices/SKILL.md` e `skills/best-practices/references/agent-principles.md`. Skill usada: `best-practices`. Nenhuma fase foi marcada como concluída sem confirmação do responsável.

> **Observação de progresso (2026-05-27, UI de resultados durante scraping):** ajustado o painel de imóveis para recolher automaticamente os filtros quando resultados parciais chegam enquanto o scraping segue em andamento, deixando os cards visíveis imediatamente; também adicionado rótulo `Loft` na lista de plataformas. Documentos obrigatórios abertos nesta execução: `PRD.md`, `SKILLS_README.md` e `skills/develop-frontend/SKILL.md`. Skill usada: `develop-frontend`. Nenhuma fase foi marcada como concluída sem confirmação do responsável.

> **Observação de progresso (2026-05-27, disparo visível do scraping imediato):** corrigido o fluxo da etapa 5 para aguardar a resposta de `POST /listings/search` antes de abrir Resultados, mantendo `start_scraping: true` e exibindo erro na própria etapa quando a chamada falha. A etapa 6 deixou de apresentar `no_cache` sem `job_id` como "Scraping em andamento", evitando spinner enganoso quando nenhum job foi criado/retornado. Documentos obrigatórios abertos nesta execução: `PRD.md`, `SKILLS_README.md` e `skills/security-threat-checklist/SKILL.md`. Skill usada: `security-threat-checklist`. Nenhuma fase foi marcada como concluída sem confirmação do responsável.

> **Observação de progresso (2026-05-27, runtime do scraping imediato com Loft):** diagnosticado que `/listings/search` criava jobs corretamente, mas o `worker-scrape-browser` ainda estava com processo antigo sem `loft` no registry, gerando `PlatformRegistryError`. Workers de scraping/prewarm foram reiniciados para recarregar configuração e código; um job reenfileirado concluiu com `101` anúncios processados e cache `complete` para as quatro plataformas. A UI de Resultados também passou a preferir o `job_id` ativo retornado pela API quando existir um job antigo falhado no estado local. Documentos obrigatórios abertos nesta execução: `PRD.md`, `SKILLS_README.md` e `skills/security-threat-checklist/SKILL.md`. Skill usada: `security-threat-checklist`. Nenhuma fase foi marcada como concluída sem confirmação do responsável.

> **Observação de progresso (2026-05-27, scraping por plataforma em cache hit autorizado):** corrigido o fluxo em que um usuário autorizado confirmava uma busca com cache completo e a API retornava apenas o cache, sem novo job por plataforma. Agora `POST /listings/search` mantém os resultados imediatos do cache, mas enfileira `listings_scrape` com `force_refresh=true` quando `start_scraping=true` e a permissão de scraping imediato está presente; `GET /zones/{zone}/listings` também expõe `job_id` ativo durante a revalidação. Documentos obrigatórios abertos nesta execução: `PRD.md`, `SKILLS_README.md` e `skills/security-threat-checklist/SKILL.md`. Skill usada: `security-threat-checklist`. Nenhuma fase foi marcada como concluída sem confirmação do responsável.
