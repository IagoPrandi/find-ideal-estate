...
describe("Step6Analysis", () => {
  ...
  it("recolhe os filtros quando resultados parciais chegam durante o scraping", async () => {
    vi.mocked(getZoneListings).mockResolvedValue({
      source: "cache",
      job_id: "listings-job-1",
      freshness_status: "no_cache",
      listings: [
        {
          property_id: "prop-parcial-1",
          platform: "loft",
          platform_listing_id: "loft-1",
          address_normalized: "Rua Guaipá, Vila Leopoldina, São Paulo, SP",
          url: "https://loft.com.br/imovel/loft-1",
          current_best_price: "5500",
          condo_fee: "700",
          iptu: "120",
          area_m2: 99,
          bedrooms: 2,
    ...
    vi.mocked(getJob).mockResolvedValue({
      ...
      result_ref: {
        scrape_diagnostics: {
          status: "scraping",
          active_platform: "loft",
          platform_order: ["loft", "quintoandar", "vivareal", "zapimoveis"],
          summary: {
            total_scraped: 1,
            platforms_completed: [],
            platforms_failed: [],
          ...
          platforms: {
            loft: { status: "scraping", scraped_count: 1 },
            quintoandar: { status: "pending" },
            vivareal: { status: "pending" },
            zapimoveis: { status: "pending" },
          },
        },
      },
    } as never);
    ...
    expect(await screen.findByText(/Rua Guaipá, Vila Leopoldina, São Paulo, SP/i)).toBeInTheDocument();
    ...
    expect(screen.getAllByText(/^Loft$/i).length).toBeGreaterThan(0);
  ...
  it("does not present no-cache inventory without a job as active scraping", async () => {
    useJourneyStore.setState((state) => ({
      ...state,
      listingsJobId: null,
    ...
    expect(await screen.findByText(/^Sem cache consolidado$/i)).toBeInTheDocument();
    expect(screen.queryByText(/^Scraping em andamento$/i)).not.toBeInTheDocument();
    expect(getJob).not.toHaveBeenCalled();
  });

  it("prefers the active job returned by listings over an older persisted failed job", async () => {
    useJourneyStore.setState((state) => ({
      ...state,
      listingsJobId: "old-failed-job",
    }));
    vi.mocked(getZoneListings).mockResolvedValue({
      source: "none",
      job_id: "new-running-job",
      freshness_status: "no_cache",
      listings: [],
      total_count: 0,
      cache_age_hours: null,
    ...
    vi.mocked(getJob).mockResolvedValue({
      ...
      result_ref: {
        scrape_diagnostics: {
          status: "scraping",
          active_platform: "loft",
          platform_order: ["loft", "quintoandar"],
          summary: {
            total_scraped: 1,
            platforms_completed: ["loft"],
            platforms_failed: [],
          },
          platforms: {
            loft: { status: "completed", persisted_count: 1 },
            quintoandar: { status: "scraping", scraped_count: 0 },
          },
        },
      },
    } as never);
    ...
    await waitFor(() => {
      expect(getJob).toHaveBeenCalledWith("new-running-job");
      expect(useJourneyStore.getState().listingsJobId).toBe("new-running-job");
    });
    expect(screen.getByText(/^Scraping em andamento$/i)).toBeInTheDocument();
    expect(screen.getByText(/Busca de imóveis:\s*30%/i)).toBeInTheDocument();
  });
