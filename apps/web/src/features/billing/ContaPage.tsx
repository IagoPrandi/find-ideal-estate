...
export function ContaPage({ onClose }: { onClose: () => void }) {
    ...
    return (
      <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/50 p-4">
        <div className="rounded-2xl bg-white p-8 text-center shadow-2xl">
          ...
          <button onClick={onClose} className="mt-4 rounded-lg bg-slate-200 px-6 py-2 text-sm font-semibold text-slate-700">Fechar</button>
        </div>
      </div>
    );
  }
  ...
  const daysUntilExpiry = () => {
    ...
    const d = Math.floor((new Date(activePlan.ends_at).getTime() - Date.now()) / 86400000);
    return d;
  };
  ...
  const currentPlanFull = plans.find((p) => p.slug === activePlan?.plan.slug);
  ...
  const usedPct = quota > 0 ? Math.min(100, Math.round(((quota - credits!.cycle) / quota) * 100)) : 0;
      ...
      <div className="w-full max-w-2xl rounded-2xl bg-white shadow-2xl">
        ...
        <div className="space-y-6 p-8">
          ...
          <div>
            ...
            <p className="font-semibold text-slate-800 mt-0.5">{authStatus.user?.email}</p>
            {isProprietario && (
          ...
          <div className="rounded-xl border border-slate-200 p-5">
            <p className="text-xs font-semibold uppercase tracking-wide text-slate-400 mb-3">Plano</p>
            {loadingPlan ? (
              ...
              <div className="flex flex-col gap-2">
                ...
                {activePlan.ends_at && (
                  <p className="text-xs text-slate-500">
                    ...
                    {expDays !== null && expDays >= 0 && (
                      <span className={`ml-2 ${expDays <= 7 ? "text-amber-600 font-semibold" : "text-slate-400"}`}>
                ...
                {shouldShowRenewalCta && (
                  <button
                    onClick={handleRenew}
                    className="mt-2 self-start rounded-lg bg-amber-500 px-4 py-1.5 text-xs font-semibold text-white hover:bg-amber-600 transition-colors"
                  >
                    Renovar via Pix
                  </button>
                )}
          ...
          {/* Credits */}
          <div className="rounded-xl border border-slate-200 p-5">
            <p className="text-xs font-semibold uppercase tracking-wide text-slate-400 mb-3">Créditos</p>
            {loadingCredits ? (
              ...
              <div className="space-y-3">
                <div className="flex items-end gap-2">
                  ...
                  {quota > 0 && <span className="text-sm text-slate-400 mb-0.5">/ {quota} cota mensal</span>}
                </div>
                {quota > 0 && (
                ...
                <div className="flex gap-4 text-xs text-slate-500">
                  <span>Ciclo: <strong>{credits.cycle}</strong></span>
                  {credits.rollover > 0 && <span>Rollover: <strong>{credits.rollover}</strong></span>}
                  {credits.legacy > 0 && <span>Avulsos: <strong>{credits.legacy}</strong></span>}
                </div>
                {credits.cycle_ends_at && (
                  <p className="text-xs text-slate-400">Ciclo encerra em: {formatDate(credits.cycle_ends_at)}</p>
                )}
              </div>
            ) : (
